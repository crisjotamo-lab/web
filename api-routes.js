...
router.get('/paypal-checkout', async (req, res) => {
  try {
    ...
    const displayProduct = String(product_name || 'Digital Product').trim() || 'Digital Product';
    const safeSuccess = escapeForJsPaypalCheckout(success_url);
    const safeCancel = escapeForJsPaypalCheckout(cancel_url);
    const safeName = escapeForJsPaypalCheckout(displayProduct);
    ...
    const htmlProduct = escapeHtmlPaypalCheckout(displayProduct);
